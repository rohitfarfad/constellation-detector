...
## 18. Q5 version 6: joint ambiguity resolution (2026-09-24)

**Latest measured status:** Request 19 reports approximately **92.3% for v5**, improving on v4's 91.790%. This supersedes §17's historical unmeasured-v5 status. The accompanying lecture slide contains anonymous private-leaderboard scores and matching tips; it does not establish the user's exact score, rank, or private result. The target is now 95%; it remains unmet. Actual prompt and attachment context are recorded in `AI_USAGE.md`.

**Preservation:** Work is on `refinement/q5-v6`. V5 code, executed notebook, calibration, diagnostics, and CSV are preserved in `outputs/q5/baseline_v5/`. Original inputs and existing Q1-Q4 edits were left untouched.

**Accepted refinement:** A single helper, `resolve_figure_pairs`, runs after existing recovery. It finds reciprocal ambiguities between two already-present figure queries: each has an image candidate within 12 pixels of the other's current location, with distinct original locations more than 24 pixels apart. Both alternatives must improve `0.4 × raw NCC + 0.6 × bandpass NCC` by at least 0.02 and retain probability at least `max(0.04, 0.5 × query-best probability)`. Fit the winning reference while excluding **both** queries and all their candidates; require at least six independent supports, the existing withheld-point and projective-horizon checks, and two distinct destination nodes within six pixels. Apply each query at most once, preferring larger combined NCC gains. This prevents unilateral moves and duplicate assignment within the pair. All coordinates come from existing computed image candidates. These gates are engineering choices, not official scorer rules.

This differs from the previously rejected unrestricted relocation: both queries must prefer the exchange, neither supplies geometric support for it, and both destinations remain distinct. The original image search, calibration, identity ranking, presence decisions, membership flags, and v5 recoveries are unchanged. No dependencies or external data are added; existing matching caches remain valid.

**Rejected experiments:** Weighted scoring alone displaced four correctly localized training queries and moved one already-false detection. Independent geometry still produced plausible wrong alternatives, including true off-figure queries. Adding an annular weighted-NCC calibration feature removed one image-only false positive but introduced another and reduced above-threshold true-candidate coverage in two scenes; it was not promoted. These experiments did not justify broad score replacement, threshold relaxation, or changes to uncertain constellation identities.

**Visual inspection and stress check:** `outputs/q5/experiments/v6_training_audit.png` shows the saturated missed training queries; `v6_pair_inspection.png` compares the proposed pair's queries, aligned old/new crops, and residuals. The competition pair remains visually ambiguous, so inspection is not evidence of ground-truth correctness. Using real training candidates, seven reciprocal errors could be constructed in Scorpius; the new rule corrected five, left two untouched, and introduced no other changes. Pisces/Taurus had no qualifying reciprocal test pairs. Original predictions on all three labeled scenes were unchanged. This stress check is not an untouched generalization estimate. Reproduce it with `outputs/q5/experiments/v6_pair_check.py`.

**Reproduction:** Run `ConstellationDetection.ipynb`, or `.venv/bin/python constellation.py --data Data --output outputs/q5/submission_v6.csv --calibration outputs/q5/calibration_v6.json`. V6 outputs use separate filenames. Completed whole-notebook, CLI, and CSV verification is recorded below.

**Completed v6 validation:** Fresh-kernel execution completed without errors in 365.52 seconds. All original training predictions and component scores are unchanged (Pisces 0.9790344, Scorpius 1.0000000, Taurus 0.9815296; mean **0.9868547**). The competition CSV changes exactly two coordinates, forming one joint reassignment: `constellation_11` queries 12 and 47. Their weighted-NCC gains are 0.02446 and 0.03968; ten other correspondences predict their distinct target nodes within 0.301 and 3.339 pixels, with maximum withheld-point error 7.356 pixels. No scene/query identifiers are coded into the algorithm. These unlabeled changes are not verified as correct.

All v5 presence decisions, membership flags, constellation names, and geometric rescues are preserved. Calibration is identical to v5. The affected scene's CLI run with no training folder reproduced its notebook row exactly. Schema checks passed for all 16 rows, 90 columns, 668 real queries, IDs/order/counts, finite bounds, flags, vocabulary, and padding. All 881 original input hashes match.

**Artifacts:** `outputs/q5/submission_v6.csv` and byte-identical `Kaggle Submissions/submission_v6.csv`; concise evidence in `outputs/q5/improvement_evidence_v6.json`. No Kaggle upload was performed. V5's user-reported approximately 92.3% remains the best measured result; v6's score and the 95% target remain unverified.


## 19. Q5 version 7: precise recovery of weak matches (2026-09-24)

**Measured baseline:** Request 20's screenshot confirms **v5 = v6 = 0.92331 public**. Equality does not identify which scenes are public or establish whether the v6 pair reassignment was correct. Per the user's instruction, work remains on `refinement/q5-v6`; v6 code, notebook, calibration, diagnostics, and CSV are preserved in `outputs/q5/baseline_v6/`. The exact prompt is in `AI_USAGE.md`. The 95% target remains unmet.

**Accepted change:** Four net lines in `recover_figure_queries` add a stricter route for otherwise rejected low-confidence candidates. The probability floor is 0.001 instead of 0.01 only when (1) at least eight correspondences from other queries support the reference transform, (2) maximum withheld-point error is at most six pixels, (3) the candidate lies within two pixels of the projected reference node, and (4) no accepted point is within 12 pixels of that node. The candidate must still retain at least half its query's best probability. Existing transform checks and six-pixel/default-0.01 recovery remain. This is a combined reliability gate, not a global threshold reduction; its numerical settings are engineering choices, not official scoring rules. No image-search, calibration, identity, or pair-reassignment changes were made.

**Experiments:**

- Visually audited all extracted reference nodes against all 48 drawings; no obvious missing-node issue justified changes (`outputs/q5/experiments/v7_reference_audit.png`).
- Refitting weak three/four-point identity hypotheses with a constrained similarity transform produced no changes on the three labeled scenes or the two weak competition winners. Excluded from production.
- Supplementing geometry with additional high-confidence accepted figure anchors produced no added detections. Excluded.
- The rejected-candidate audit showed that many weak matches hit already-claimed stars, including known absent training queries; relaxing the floor without an occupancy check is unsupported.
- A diagnostic seeded refinement at **training ground-truth locations only**, to test whether broader search could resolve the two known misses. Pisces query 22 still reached only 0.03460 maximum held-out confidence; Taurus query 18 had no valid near-truth ECC fit. These ground-truth seeds are diagnostic only and never enter candidate caches, production matching, or submissions.

**Noise stress check:** Added Gaussian noise with standard deviations 5, 15, and 25 to every released training query, clipped to uint8, and recomputed image evidence at the existing candidate warps. Held-one-scene-out calibration was retained; this isolates recovery and does not test a new image search. Of 348 variants, 192 had maximum confidence below 0.04 and were evaluated for recovery, including 124 absent variants. The new path added one correct Scorpius figure detection and no new false detections in this check. These are correlated variants of the same three scenes, not independent generalization evidence. Script and results: `outputs/q5/experiments/v7_recovery_check.py`, `v7_recovery_noise_check.json`.

**Reproduction:** Run `ConstellationDetection.ipynb`, or `.venv/bin/python constellation.py --data Data --output outputs/q5/submission_v7.csv --calibration outputs/q5/calibration_v7.json`. Completed validation is recorded below.

**Completed v7 validation:** The notebook executed from a fresh kernel without errors in 344.20 seconds. All original training predictions and component scores are unchanged (Pisces 0.9790344, Scorpius 1.0000000, Taurus 0.9815296; mean **0.9868547**). Calibration is unchanged. The affected-scene CLI run with saved calibration and no training directory reproduces the notebook row exactly. The 16-row/90-column CSV passes all schema, ID/order/count, 668-query, coordinate, flag, vocabulary, and padding checks. All 881 original input hashes match.

The only new prediction is `constellation_12` query 15 at `(1525.33, 1289.69, 1)`, computed from an existing image candidate. Eight other correspondences support it; its reference-node distance is 1.743 pixels and maximum withheld-point error is 4.321 pixels. Every previously accepted v6 coordinate/flag and every identity is unchanged. The saturated query is visually ambiguous (`outputs/q5/experiments/v7_recovery_inspection.png`); geometry, not visual certainty, supports the addition. Its correctness and Kaggle benefit remain unverified.

**Artifacts:** `outputs/q5/submission_v7.csv` and byte-identical `Kaggle Submissions/submission_v7.csv`; concise evidence in `outputs/q5/improvement_evidence_v7.json`. No Kaggle upload was performed. The best measured public score remains **0.92331** for v5/v6; v7 and the 95% target are unmeasured.
