...
def analyze_scene(folder, cache):
    """Content-addressed caches store computed evidence, never training answers."""
...
def candidate_probabilities(matches, calibration):
    if not matches:
        return np.array([])
    features = np.array([m['features'] for m in matches])
    ...
    for path in sorted(Path(folder).glob('*_pattern.png')):
        ...
        for i in range(1, count):
            if stats[i, cv2.CC_STAT_AREA] >= 4:
                ...
                nodes.extend(inside if len(inside) > 1 else [centers[i]])
...
def identify_constellation(points, owners, probabilities, patterns, shape=(3000, 3000)):
    ...
    for name, nodes in patterns.items():
        if len(nodes) < 4:
            continue  # Two/three unlabeled points cannot establish a unique affine identity.
        proposals = []
        ...
        for _, matrix in proposals:
            ...
            finalists.append(dict(name=name, score=score, support=len(src),
                error=float(np.median(errors)), matrix=matrix.tolist(),
...
def verify_constellations(ranking, patterns, image):
    ...
    for model in ranking:
        ...
        # Discount three fitted point pairs and the extra opportunities in large diagrams.
        independent = max(0, model['support']-3)/model['support']
        model['score'] *= independent * ratio / np.sqrt(len(nodes))
    return sorted(ranking, key=lambda r: r['score'], reverse=True)


def predict_scene(evidence, calibration, patterns, image, threshold=.20):
    ...
    for i, (matches, probs) in enumerate(zip(evidence, probabilities)):
        ...
        for j in np.argsort(probs)[::-1][:2]:
            if probs[j] >= max(.04, .6*probs.max()):
                points.append([matches[j]['x'], matches[j]['y']])
                owners.append(i)
                confidence.append(float(probs[j]))
    ...
    chosen = {}
    if winner and winner['support'] >= 4:
        ...
        for j in associated:
            chosen[owners[j]] = alternatives[j]
    prediction = []
    for i, (matches, probs) in enumerate(zip(evidence, probabilities)):
        if not len(probs):
            prediction.append(-1)
